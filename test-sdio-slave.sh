...
if [ -n "${FLEXE_SDIO_SLAVE_BUILD_DIR:-}" ]; then
    fixture_build=$FLEXE_SDIO_SLAVE_BUILD_DIR
    mkdir -p -- "$fixture_build"
else
    fixture_build=$(mktemp -d "${TMPDIR:-/tmp}/flexe-sdio-slave.XXXXXX")
...
if [ -n "$arduino_config" ]; then
    "$arduino_cli" --config-file "$arduino_config" compile \
        ...
        "$repo_dir/tests/fixtures/sdio_slave"
else
    "$arduino_cli" compile \
        --fqbn "$fqbn" \
        --build-path "$fixture_build" \
...
cmake --build "$host_build" --target flexe-sdio-slave-test -j
"$host_build/flexe-sdio-slave-test" \
    "$fixture_build/sdio_slave.ino.bin" \
    "$fixture_build/sdio_slave.ino.elf"
"$host_build/flexe-sdio-slave-test" --no-jit \
    "$fixture_build/sdio_slave.ino.bin" \
    "$fixture_build/sdio_slave.ino.elf"
