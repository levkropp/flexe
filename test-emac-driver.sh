...
if [ -n "${FLEXE_EMAC_DRIVER_BUILD_DIR:-}" ]; then
    fixture_build=$FLEXE_EMAC_DRIVER_BUILD_DIR
    mkdir -p -- "$fixture_build"
else
    fixture_build=$(mktemp -d "${TMPDIR:-/tmp}/flexe-emac-driver.XXXXXX")
...
if [ -n "$arduino_config" ]; then
    "$arduino_cli" --config-file "$arduino_config" compile \
        ...
        "$repo_dir/tests/fixtures/emac_driver"
else
    "$arduino_cli" compile \
        --fqbn "$fqbn" \
        --build-path "$fixture_build" \
...
"$host_build/flexe-emac-hal-test" --driver \
    "$fixture_build/emac_driver.ino.bin" \
    "$fixture_build/emac_driver.ino.elf"
"$host_build/flexe-emac-hal-test" --no-jit --driver \
    "$fixture_build/emac_driver.ino.bin" \
    "$fixture_build/emac_driver.ino.elf"
