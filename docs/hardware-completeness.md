...
| S3 I2S v2 | Partial (MMIO and timed GDMA stream) | `tests/test_i2s_v2.c`; `scripts/check-s3-idf-i2s-std.sh` runs ESP-IDF 5.3.2's unmodified standard-mode driver in interpreter and JIT, streams a four-descriptor circular ring, wakes blocking writers through GDMA EOF interrupts, verifies 48 kHz/stereo/16-bit metadata and GPIO-matrix routes, and leaves zero unsupported accesses | Standard master TX is driver-gated and host RX transport is unit-tested. PDM, slave/external-clock behavior, wire-level BCLK/WS/data edges, stock-driver RX, port 1, underrun/overrun latency, and calibrated audio timing remain unverified. |
| S3 GDMA | Partial (MMIO) | `tests/test_crypto.c` checks controller-wide clock/arbitration/AHB-reset configuration, finite chained TX/RX descriptors, ownership/writeback, errors, and per-channel level interrupts on both cores; `tests/test_i2s_v2.c` checks descriptor-at-a-time circular streams; GP-SPI and stock I2S gates exercise two independent peripheral consumers | Full priority/arbitration behavior and other streaming consumers such as LCD/camera/continuous ADC remain unverified. |
