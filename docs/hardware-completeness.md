...
| S3 LCD_CAM i80 TX | Partial (MMIO/GDMA and host panel bus) | `tests/test_lcd_cam.c`; `scripts/check-s3-idf-lcd-i80.sh` runs ESP-IDF 5.3.2's unmodified i80 driver twice in each engine through SYSTEM reset/clock, GPIO routing, dummy bootstrap, polling command/parameter writes, queued color descriptors, the shared interrupt ISR, callbacks, and teardown with zero unsupported accesses | Command and DMA-controlled i80 output, 8/16-bit stride, and bit/byte order are modeled. Finite non-DMA data counts, RGB scanout, camera capture, color conversion, pad-edge timing, and electrical behavior remain diagnostic or unverified. |
...
| S3 GDMA | Partial (MMIO) | `tests/test_crypto.c` checks controller-wide clock/arbitration/AHB-reset configuration, finite chained TX/RX descriptors, ownership/writeback, errors, and per-channel level interrupts on both cores; `tests/test_i2s_v2.c` and `tests/test_lcd_cam.c` check streaming consumers; GP-SPI, stock I2S, and stock LCD i80 gates exercise independent consumers plus I2S trigger IDs 3/4 and LCD trigger 5 | Full priority/arbitration behavior and other streaming consumers such as camera/continuous ADC remain unverified. |
...
page while implementing only the i80 transmit engine. Target data supplies
...
interrupt. Camera start, RGB scanout, color conversion, and finite transfers
that would stop part-way through a DMA descriptor stay diagnostic instead of
