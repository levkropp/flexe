...
| S3 timers, watchdogs, RTC | Partial (MMIO) | `tests/test_systimer.c`, `tests/test_timer_group.c`, `tests/test_rtc_cntl.c` (masked RTC configuration, supply, analog-control and power/isolation-domain resolution, fault-injection selection, CPU-follow, sleep/wake, and stall enable), ESP-IDF cross-core, restart, zero-unsupported native timer, GPIO light/deep-sleep, and touch light-sleep gates | Timer, EXT0/EXT1, and registered touch-v2 light-sleep wake work; OPTIONS0, ANA_CONF, SLP_REJECT_CONF, SDIO/BIAS/drive configuration, FIB reset-source selection, digital-pad/global isolation, digital domains, and RTC-local domains have architectural state, with selected consumers connected. Brownout voltage detection/reset, touch deep-sleep retention, ULP wake, analog transition timing, and other reset causes remain unsupported. |
...
| S3 capacitive touch v2 | Partial (MMIO plus host samples) | `tests/test_touch_v2.c`; `scripts/check-s3-idf-touch.sh` runs ESP-IDF 5.3.2's public touch driver twice in each engine through initialization, timer scans, raw/benchmark reads, threshold transitions, RTC active/inactive interrupts, ISR-to-task notification, touch-triggered light-sleep wake, and teardown with zero unsupported accesses | Upward-count threshold scanning, sleep-pad threshold wake, and software-visible filter/approach/DAC state are modeled in functional fast mode. Calibrated charge/discharge cadence, IIR/jitter/debounce timing, approach sensing, timeout behavior, deep-sleep touch-state retention, ULP integration, and electrical capacitance remain unsupported. |
...
so this is functional wake ordering, not calibrated sleep duration. Touch-v2
light-sleep wake is modeled through the device registration path described
below; deep-sleep touch retention, ULP wake, and analog voltage-transition
timing are not modeled. Unregistered sources do not synthesize a wake.
...
arming touch plus a timer fallback, entering light sleep, and waking from a
second host-driven threshold crossing. The RTC controller accepts the touch
trigger through a generic device wake-source registry and reports both
`ESP_SLEEP_WAKEUP_TOUCHPAD` and pad 4 through the public APIs. Its image
SHA-256 is
`02103cf54ec367268861e28cb5f8211c07cbe09a4b04b3f51d192fe1698141cd` and
matching ELF SHA-256 is
`b3ea2e3d5d9dbd9e74fe55ba7818ce01734cbf82d60ee0b207faf13cd9afa9ed`.
...
capacitance, approach/timeout transitions, or deep-sleep touch-state
retention.
...
160 MHz source and 80 MHz capture clocks, interrupt sources, twelve output
signals, eighteen sync/fault/capture inputs, and their clock/reset owners.
The shared model therefore has no firmware name, fixed PC, or S3-address
branch. Each group retains three independent timers and operators, two
generators per operator, compare shadow updates, timer/generator actions,
continuous and event force, dead-time/carrier configuration, brake state,
capture timestamps, and level interrupts. S3 output and input routes use the
live target GPIO matrix. A routed pad is sampled at its current virtual time,
while individual PWM edges enter the scheduler only when a matrix-input
watcher or GPIO interrupt can observe them; unobserved aggregate PWM does not
impose a per-edge execution cost.

The native `tests/fixtures/s3_idf_mcpwm` project uses ESP-IDF 5.3.2's public
`mcpwm_prelude` API without emulator hooks. It runs one 1 kHz generator in
each hardware group, receives timer-empty and compare callbacks from native
interrupt sources 31 and 32, changes the compare points from 250/750 to
400/600 ticks, verifies both GPIO loopback levels under continuous force,
captures positive and negative GPIO6 edges through the group-0 capture ISR,
then deletes every driver object. Two runs in each engine are byte-identical,
the JIT retires native instructions, and all four runs report zero unsupported
MMIO. Its pinned application image SHA-256 is
`002d199595041406f6b9af3aaf4e014ced79519213b2bc896e27344aeb5810f7`;
the matching ELF SHA-256 is
`584a464b02524583364ed53df445eb791fd8788677a7887701630caaa152f8a7`.
Rebuild and replay it with:

```sh
S3_ROM_ELF=/path/to/esp32s3_rev0_rom.elf \
  ./scripts/build-s3-idf-fixture.sh --check mcpwm
```

This establishes functional driver behavior and guest-time ordering, not
electrical or cycle accuracy. Dead-time and carrier settings are exposed to
...
Target data supplies
its base address, SYSTEM gate/reset, shared interrupt source 24, GDMA trigger
5, version value, and all LCD GPIO-matrix producers. A transaction preserves
the command and dummy phases, drains each finite GDMA descriptor in order,
applies the selected 8/16-bit data and bit/byte ordering, updates GDMA
ownership/completion state, then raises the level-sensitive LCD `TRANS_DONE`
interrupt. Host camera injection accepts one complete DMA descriptor at a
time, applies the camera bit/byte-order controls, preserves a programmed
camera EOF interval across descriptor boundaries, and raises VSYNC separately
through the shared interrupt source. This lets a host sensor model interleave
guest ISR work without coalescing multiple camera events. RGB scanout, color
conversion, and finite LCD transfers that would stop part-way through a DMA
descriptor stay diagnostic instead of returning fake success.

The native `tests/fixtures/s3_idf_lcd_i80` project uses ESP-IDF 5.3.2's public
`esp_lcd_new_i80_bus` and panel-IO APIs without an LCD transaction hook. It
performs the driver's dummy interrupt bootstrap, a parameter transaction, a
command-only transaction, two queued color transfers and ISR callbacks, then
deletes the IO and bus. Two runs in each engine are byte-identical, the JIT
retires native instructions, and all four runs report zero unsupported MMIO.
Its pinned application image SHA-256 is
`ce3cabb330b600161d3add5b753a1a94df513833ee98739e340122309f27d63f`;
the matching ELF SHA-256 is
`31940ce06ae7783646de249d811d4ef23429b22b23626beb4e3169d1b7e8fc19`.
Rebuild and replay it with:
