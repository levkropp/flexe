...
| S3 image, ROM, and flash | `tests/test_loader.c`, `tests/test_spi_mem.c`; NerdMiner 1.8.3 factory image mounts SPIFFS and serves its own configuration page over a symbol-resolved host socket bridge; loader tests preserve NOR across reset | Automate the interactive S3 production gate and test guest filesystem reads/writes across reset. |
...
roughly 7,000 unsupported accesses (mostly RF/PHY) remain visible, and the
RTC watchdog currently resets this run after several seconds. The subsequent
boot fails PSRAM initialization. These limits prohibit a production-support
claim. ROM images and third-party firmware binaries are not copied into this
repository.
...
The app/ROM binaries are external inputs. To reproduce the HTTP check, run
the emulator without `-q`, read the `[wifi] bind(slot 48) firmware port 80 →
host 127.0.0.1:<port>` line on stderr, then request
`curl -H 'Host: 192.168.4.1' http://127.0.0.1:<port>/wifi` from another
terminal while the guest runs. The response is generated by the firmware's
WebServer and WiFiManager. The socket descriptor base `48` is this Arduino
build's ESP-IDF configuration, not a universal S3 constant; other SDK
configurations need discovery before this bridge can claim support. The next
gate should automate the request/response and investigate the watchdog reset
and PSRAM failure without suppressing unsupported PHY diagnostics.
