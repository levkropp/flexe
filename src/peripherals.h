...
/* Internal controller lookup used by the board/device attachment layer. */
 ...
 * S3's global force-hold source is cleared by ROM boot, so only individually
 * held pads are carried across the rebuild. */
typedef struct {
...
/* True once firmware or a modeled watchdog has requested a system reset;
