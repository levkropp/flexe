...
if [ -n "${FLEXE_RTC_I2C_BUILD_DIR:-}" ]; then
    fixture_build=$FLEXE_RTC_I2C_BUILD_DIR
    mkdir -p -- "$fixture_build"
else
    fixture_build=$(mktemp -d "${TMPDIR:-/tmp}/flexe-rtc-i2c.XXXXXX")
...
if [ -n "$arduino_config" ]; then
    "$arduino_cli" --config-file "$arduino_config" compile \
        ...
        "$repo_dir/tests/fixtures/rtc_i2c"
else
    "$arduino_cli" compile \
        --fqbn "$fqbn" \
        --build-path "$fixture_build" \
...
cmake --build "$host_build" --target flexe-rtc-i2c-test -j
"$host_build/flexe-rtc-i2c-test" \
    "$fixture_build/rtc_i2c.ino.bin" \
    "$fixture_build/rtc_i2c.ino.elf"
"$host_build/flexe-rtc-i2c-test" --no-jit \
    "$fixture_build/rtc_i2c.ino.bin" \
    "$fixture_build/rtc_i2c.ino.elf"
