...
if [ -n "${FLEXE_RTCGPIO_BUILD_DIR:-}" ]; then
    fixture_build=$FLEXE_RTCGPIO_BUILD_DIR
...
"$arduino_cli" compile \
    ...
    "$repo_dir/tests/fixtures/rtc_gpio"

cmake --build "$host_build" --target flexe-rtc-gpio-test -j
"$host_build/flexe-rtc-gpio-test" \
    "$fixture_build/rtc_gpio.ino.bin" \
    "$fixture_build/rtc_gpio.ino.elf"
"$host_build/flexe-rtc-gpio-test" --no-jit \
    "$fixture_build/rtc_gpio.ino.bin" \
    "$fixture_build/rtc_gpio.ino.elf"
