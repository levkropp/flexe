...
if [ -n "${FLEXE_GUESTCLOCK_BUILD_DIR:-}" ]; then
    fixture_build=$FLEXE_GUESTCLOCK_BUILD_DIR
...
"$arduino_cli" compile \
    ...
    "$repo_dir/tests/fixtures/guest_clock"

cmake --build "$host_build" --target flexe-guest-clock-test -j
"$host_build/flexe-guest-clock-test" \
    "$fixture_build/guest_clock.ino.bin" \
    "$fixture_build/guest_clock.ino.elf"
"$host_build/flexe-guest-clock-test" --no-jit \
    "$fixture_build/guest_clock.ino.bin" \
    "$fixture_build/guest_clock.ino.elf"
