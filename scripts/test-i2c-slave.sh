...
if [ -n "${FLEXE_I2CSLAVE_BUILD_DIR:-}" ]; then
    fixture_build=$FLEXE_I2CSLAVE_BUILD_DIR
...
"$arduino_cli" compile \
    ...
    "$repo_dir/tests/fixtures/i2c_slave"

cmake --build "$host_build" --target flexe-i2c-slave-test -j
"$host_build/flexe-i2c-slave-test" \
    "$fixture_build/i2c_slave.ino.bin" \
    "$fixture_build/i2c_slave.ino.elf"
"$host_build/flexe-i2c-slave-test" --no-jit \
    "$fixture_build/i2c_slave.ino.bin" \
    "$fixture_build/i2c_slave.ino.elf"
